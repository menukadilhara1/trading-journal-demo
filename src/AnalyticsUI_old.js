...
// Γ£à Maps local trade object -> backend payload (includes optional analytics fields)
...
export default function AnalyticsUI({ isActive, startingBalance = 0, trades: propTrades = [] }) {
  ...
  // Γ£à USE PROP TRADES (Source of truth from App.js)
  const trades = propTrades;

  // Γ£à FIX: Add refresh trigger to reload data without full page refresh
  const [refreshTrigger, setRefreshTrigger] = useState(0);

  // Γ£à FIX: Auto-refresh when user switches back to this tab/page
  useEffect(() => {
  ...
  // Γ£à Assumption: backend returns an array of trades from GET /api/trades
  // If your endpoint is different, change ONLY this URL.
  // Γ£à Removed internal fetching logic to rely on synced props from App.js
  // This ensures instant updates when toggles change in the parent.

  // (Refresh trigger kept if needed for other side effects, but not fetching)
  ...
  function normalizeTrade(t) {
    ...
    // Γ£à Priority check: explicit isBreakeven flag (from App.js local state)
    let outcome = "";
    ...
    // Γ£à If Breakeven, force PnL to 0 for all charts/stats
    if (outcome === 'be') {
      pnlNum = 0;
    }
    ...
    const durationMin = Number(t.duration_min ?? t.duration ?? 0);

    // emotion label (string)
    const emotion = (t.emotion ?? t.mood ?? "").toString();

    // date
    // Γ£à Heatmap must use the REAL trade date only.
  ...
  // Γ£à Dynamic Filters: Derive unique Instruments and Sessions from actual data
  ...
  const stats = useMemo(() => {
    ...
    // Γ£à Exclude breakeven trades from Win/Loss counts even if they have PnL
    ...
    const netPnl = totalWinPnl - totalLossPnlAbs; // Γ£à Restored missing calc
    ...
    return {
      ...
      biggestWin,
      biggestLoss,
      netPnl,     // Γ£à Return it
      winsCount,  // Γ£à Return it (used in win ratio card subtext?)
      lossesCount // Γ£à Return it
    };
  ...
  const emotionData = useMemo(() => {
    ...
    return rows.map(r => {
      ...
      const emoji =
        lower.includes("conf") ? "≡ƒñæ" :
          lower.includes("good") ? "≡ƒÿÇ" :
            lower.includes("meh") ? "≡ƒÿæ" :
              lower.includes("bad") ? "≡ƒÿÆ" :
                lower.includes("worst") ? "≡ƒÖü" :
                  r.name === "Unspecified" ? "≡ƒ½Ñ" : "≡ƒÖé";
      return { ...r, name: `${emoji} ${r.name}` };
    });
  }, [filteredTrades]);

  // Γ£à TradesBySymbol dataset (REAL)
  ...
  // Γ£à MoodCorrelation dataset (REAL)
  ...
  // Γ£à FIXED: Weekly Heatmap Data - Resets each month, only shows data for days that have passed
  const weeklyHeatmapData = useMemo(() => {
    ...
    // Γ£à Get current month and build weeks ONLY for current month
    ...
    // Γ£à Fill from trades - ONLY trades from current month
    for (const t of filteredTrades) {
    ...
    // Γ£à Clean up: Keep structure for UI
      ...
      {!loading && !loadError && (
        <>
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="text-2xl font-bold text-[#0F172A]">Analytics</div>
              <div className="text-sm text-[#64748B]">Your performance, visualized</div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                className="px-3 py-2 rounded-lg border border-[#E2E8F0] bg-white text-[#0F172A] text-sm font-semibold hover:shadow-sm flex items-center gap-2 disabled:opacity-50"
              ...
              <button
                type="button"
                className="px-3 py-2 rounded-lg border border-[#E2E8F0] bg-white text-[#0F172A] text-sm font-semibold hover:shadow-sm flex items-center gap-2"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 rounded-lg border border-[#E2E8F0] text-[#171717] bg-white focus:border-[#2563EB] focus:outline-none"
                >
          ...
          {stats.ungradedTrades > 0 && (
            <div className="bg-white rounded-xl border border-[#F59E0B]/30 p-4 text-sm text-[#0F172A]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KpiCard
              ...
              title="Profit Factor"
              value={stats.profitFactor >= 999 ? "Γê₧" : stats.profitFactor.toFixed(2)}
              sub="Wins / Losses"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* RIGHT */}
            <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
              <div className="text-base font-bold text-[#171717] mb-4">
                Mood ΓåÆ Performance
              </div>
      ...
      {/* CUSTOM DATE PICKER MODAL */}
      {
        showDatePicker && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-[#E2E8F0] overflow-hidden animate-in fade-in zoom-in duration-200">
              <div className="px-6 py-4 border-b border-[#E2E8F0] flex justify-between items-center bg-[#F8FAFC]">
                ...
                <button
                  ...
                  className="text-[#64748B] hover:text-[#EF4444] transition-colors p-1 rounded-full hover:bg-red-50"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 space-y-5">
                <div className="space-y-2">
                  <label className="text-xs font-bold text-[#64748B] uppercase tracking-wider block">Start Date</label>
                  <input
                    type="date"
                    value={tempCustomRange.start}
                    onChange={(e) => setTempCustomRange({ ...tempCustomRange, start: e.target.value })}
                    className="w-full px-4 py-3 rounded-xl border border-[#E2E8F0] text-[#0F172A] font-medium focus:ring-2 focus:ring-[#2563EB]/20 focus:border-[#2563EB] outline-none transition-all"
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-xs font-bold text-[#64748B] uppercase tracking-wider block">End Date</label>
                  <input
                    type="date"
              ...
              <div className="px-6 py-4 bg-[#F8FAFC] border-t border-[#E2E8F0] flex justify-end gap-3">
                <button
                  onClick={() => setShowDatePicker(false)}
                  className="px-5 py-2.5 rounded-xl text-sm font-semibold text-[#64748B] hover:bg-white hover:text-[#0F172A] border border-transparent hover:border-[#E2E8F0] transition-all"
                >
                  Cancel
                </button>
                <button
...
// KPI Card Component
function KpiCard({ icon, title, value, sub, valueColor = 'text-[#171717]' }) {
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-5 hover:shadow-md hover:-translate-y-0.5 hover:border-[#2563EB]/40 transition-all">
      ...
      <div className="text-sm text-[#64748B] mt-1">{sub}</div>
    </div>
  );
}
...
function DirectionDonutCard({ title, profit, wins, losses, winRate, totalWins, totalLosses, totalTrades, isOverall }) {
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E2E8F0] p-6">
      <div className="text-base font-bold text-[#171717] mb-4">{title}</div>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <PieChart>
          <Pie
            ...
            dataKey="value"
            cx="50%"
            cy="72%"           // ≡ƒæê pushes donut down onto the divider
            startAngle={180}
            endAngle={0}
            innerRadius={70}   // ≡ƒæê thicker donut
            outerRadius={90}
            paddingAngle={0}
            cornerRadius={6}
          >
      ...
      {isOverall ? (
        <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-[#E2E8F0]">
          <div className="text-center">
            <div className="text-2xl font-bold text-[#10B981]">{winRate.toFixed(1)}%</div>
            <div className="text-xs text-[#64748B] mt-1">Win Rate</div>
          </div>
          <div className="text-center">
            <div className="text-sm text-[#64748B]">
              <span className="text-[#10B981] font-bold">{wins}W</span> / <span className="text-[#EF4444] font-bold">{losses}L</span>
            </div>
            <div className="text-xs text-[#64748B] mt-1">Record</div>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-3 gap-2 mt-4 pt-4 border-t border-[#E2E8F0]">
          <div className="text-center">
            <div className="text-sm font-bold text-[#10B981]">Wins ({wins})</div>
            <div className="text-xs text-[#64748B]">${totalWins.toFixed(0)}</div>
          </div>
          <div className="text-center">
            <div className="text-sm font-bold text-[#171717]">{winRate.toFixed(0)}%</div>
            <div className="text-xs text-[#64748B]">Win Rate</div>
          </div>
          <div className="text-center">
            <div className="text-sm font-bold text-[#EF4444]">Losses ({losses})</div>
            <div className="text-xs text-[#64748B]">${totalLosses.toFixed(0)}</div>
          </div>
        </div>
      )}
    </div>
